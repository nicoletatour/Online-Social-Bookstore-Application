...
public class BookOfferController {
    ...
    public String listBookOffers(Model model) {
        ...
        List<BookDTO> bookOffersDtos = bookOfferService.listBookOffersBusinessLogic(username);
    ...
    public String saveBookOffer(@ModelAttribute("offer") BookDTO bookOfferDto) {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String username = authentication.getName();

        bookOfferService.saveBookOfferBusinessLogic(username, bookOfferDto);
